...
def new_session(model_name: str, bucket_name: str = None, blob_name: str = None) -> BaseSession:
    ...
    if bucket_name is not None and blob_name is not None:
        if not path.exists():
            with redirect_stdout(sys.stderr):
                download_from_gcp(bucket_name, blob_name, str(path))
        else:
...
def download_from_gcp(bucket_name, blob_name, destination_path):
    storage_client = storage.Client()
    bucket = storage_client.get_bucket(bucket_name)
    blob = bucket.blob(blob_name)
    blob.download_to_filename(destination_path)
    print(f'Blob {bucket_name}/{blob_name} downloaded to {destination_path}')
    return